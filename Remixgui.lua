...
local BgImageID = "rbxassetid://98360849741395"
...
if KeySystem then
    ...
    local Background = Create("ImageLabel", KeyMenu, {
    ...
    CreateTween(UIScale, "Scale", 0, 0.4, true)
    KeyMenu:Destroy()
end
  local TweenService = game:GetService("TweenService")
...
local Menu = Create("Frame", ScreenGui, {
    ...
    Position = UDim2.new(0.5, -500/2, -1, -270/2),
    Active = true,
    Draggable = true,
...
Minimize_BTN.MouseButton1Click:Connect(function()
    ...
    CreateTween(Menu, "Size", IsMinimized and UDim2.new(0, 500, 0, 25) or UDim2.new(0, 500, 0, 270), 0.25, false)
    CreateTween(Menu, "Position", IsMinimized and UDim2.new(0.5, -250, 0.5, -12.5) or UDim2.new(0.5, -250, 0.5, -135), 0.25, false)
end)
...
local function CreateClose()
    ...
    CreateTween(CloseMenu, "BackgroundTransparency", 0, 0.2, false)
    CreateTween(CloseMenu, "Size", UDim2.new(0.7, 0, 0.7, 0), 0.25, false)  

    Cancel.MouseButton1Click:Connect(function()    
        CreateTween(CloseMenu, "BackgroundTransparency", 1, 0.3, false)    
        CreateTween(CloseMenu, "Size", UDim2.new(0,0,0,0), 0.25, true)    
        CloseGui:Destroy()    
    end)  
...
CreateTween(Menu, "Position", UDim2.new(0.5, -250, 0.5, -135), 0.55, false)
...
function CreateTween(Object, Property, Goal, Time, Reverses)
    local TweenInfoData = TweenInfo.new(Time, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, Reverses or false, 0)
    local Tween = TweenService:Create(Object, TweenInfoData, {[Property] = Goal})
    Tween:Play()
end
...
function MinimizeButton(Configs)
    ...
    Button.MouseButton1Click:Connect(function()
        if minimize then
            minimize = false
            Menu.Visible = true
            CreateTween(Button, "Rotation", 0, 0.25, false)
            ...
            minimize = true
            CreateTween(Button, "Rotation", 180, 0.25, false)
...
function MakeTab(Configs)
    ...
    local Indicator = Create("Frame", Frame, {
    ...
    TextButton.MouseButton1Click:Connect(function()
        ...
        for _, frame in pairs(ScrollBar:GetChildren()) do
            if frame:IsA("Frame") and frame:FindFirstChild("TextLabel") and frame.TextLabel ~= TextLabel then
                ...
                if frame:FindFirstChild("Indicator") then
                    frame.Indicator:TweenSize(UDim2.new(0, 0, 0, 2), "Out", "Quad", 0.2, true)
                    task.delay(0.2, function()
                        frame.Indicator.Visible = false
                    end)
                end
            end
        end
        ...
        Indicator:TweenSize(UDim2.new(0, 10, 0, 2), "Out", "Quad", 0.3, true, function()
            Indicator:TweenSize(UDim2.new(0, 0, 0, 2), "Out", "Quad", 0.2, true, function()
  ...
  function AddTextBox(parent, Configs)
    ...
    }) Corner(DropFrame) Stroke(DropFrame)

    local UIList = Instance.new("UIListLayout", DropFrame)
    UIList.Padding = UDim.new(0, 2)

    local NumberList = {123, 456, 789, 101, 202}
    ...
    local function Open(size)
        CreateTween(DropFrame, "Size", UDim2.new(0, 120, 0, size), 0.2, true)
    end

    local function AddOption(text, image)
        local Opt = Create("Frame", DropFrame, {
            Size = UDim2.new(1, 0, 0, 30),
            BackgroundColor3 = Configs_HUB.Cor_Options
        })

        local Img = Create("ImageLabel", Opt, {
            Size = UDim2.new(0, 25, 0, 25),
        ...
        local Txt = Create("TextButton", Opt, {
            BackgroundTransparency = 1,
            Text = text,
            TextColor3 = Configs_HUB.Cor_Text,
    ...
    local function GenerateResults(txt)
        ClearList()
        if txt == "" then Open(0) return end

        local Results = {}
        if tonumber(txt) ~= nil then
            for _, num in ipairs(NumberList) do
                if tostring(num):sub(1, #txt) == txt then
                    table.insert(Results, {txt = tostring(num), img = "rbxassetid://0"})
                end
            end
        else
            for _, plr in ipairs(game:GetService("Players"):GetPlayers()) do
                if plr.Name:lower():sub(1, #txt) == txt:lower() then
                    local thumb = game:GetService("Players"):GetUserThumbnailAsync(plr.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size100x100)
                    table.insert(Results, {txt = plr.Name, img = thumb})
                end
            end
        end

        if #Results == 0 then Open(0) return end
        for _, data in ipairs(Results) do
            AddOption(data.txt, data.img)
        end
        Open(#Results * 32)
    end

    TextBox:GetPropertyChangedSignal("Text"):Connect(function()
        GenerateResults(TextBox.Text)
    end)
        ...
        if ClickConter == 0 or tick() - time > 5 then
            ...
            task.spawn(function()
                pcall(function()
                    ...
                    elseif game:FindFirstChild("OpenBrowserWindow") then
                        game:OpenBrowserWindow(DiscordLink)
                    else
                        local HttpService = game:GetService("HttpService")
                        local encoded = HttpService:UrlEncode(DiscordLink)
                        game:OpenBrowserWindow("https://www.google.com/search?q=" .. encoded)
                    end
                end)
            end)
            ClickConter = ClickConter + 1
            JoinButton.Text = "Opening..."
